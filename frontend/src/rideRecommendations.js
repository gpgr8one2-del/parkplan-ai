...
function buildReason(ride, scoreParts) {
  const reasons = [];
  ...
  if (scoreParts.baseScore >= 85) {
    reasons.push("high-priority attraction");
  }

  if (scoreParts.trendModifier > 0) {
    reasons.push("strong guest demand");
  }

  if (scoreParts.weatherModifier > 0) {
...
export function getNextBestRides({ parkId, rides = [], weather = null }) {
  ...
  const waitOnThis =
    rides
      .filter((ride) => ride.isOpen && getBaseScore(parkId, ride.name) >= 85)
